<template>
  <el-drawer v-model="visible" title="应收详情" size="540px">
    <div v-if="row" class="detail">
      <div class="detail__head">
        <div class="detail__supplier">{{ row.customer?.name || '-' }}</div>
      ...
      <el-descriptions :column="2" border size="default" class="mt">
        ...
        <el-descriptions-item label="已收金额">¥{{ formatMoney(row.received_amount) }}</el-descriptions-item>
        ...
        <el-descriptions-item label="收款条件">{{ row.payment_term || '-' }}</el-descriptions-item>
        <el-descriptions-item label="关联项目">{{ row.project?.name || '-' }}</el-descriptions-item>
      </el-descriptions>
      <div class="detail__section-title">收款进度</div>
...
// v0.3.25 抽自 finance/Receivable.vue:227-247
