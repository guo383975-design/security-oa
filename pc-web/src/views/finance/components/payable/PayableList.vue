<template>
    ...
    <div v-for="row in list" :key="row.id" class="pcard" :class="`pcard--${row.status}`">
      <div class="pcard__head">
        <div class="pcard__title">
          <div class="pcard__supplier">{{ row.supplier?.name || '-' }}</div>
      ...
      <div class="pcard__amount">
        <div class="pcard__amount-label">应付金额</div>
      ...
      <div class="pcard__progress">
        <div class="pcard__progress-row">
          ...
          <span>未付 <span :class="Number(row.remaining_amount) > 0 ? 'text-danger' : 'text-success'">¥{{ formatMoney(row.remaining_amount) }}</span></span>
        </div>
      ...
      <div class="pcard__meta">
        ...
        <div v-if="row.payment_term" class="pcard__meta-item">
          ...
          <span>付款条件：{{ row.payment_term }}</span>
        </div>
      </div>
      <div class="pcard__actions">
        <el-button link type="primary" size="small" @click="emit('pay', toPayable(row))" :disabled="Number(row.remaining_amount) <= 0"><el-icon><CreditCard /></el-icon>登记付款</el-button>
        <el-button link type="primary" size="small" @click="emit('edit', toPayable(row))">编辑</el-button>
        <el-button link type="info" size="small" @click="emit('detail', toPayable(row))">详情</el-button>
        <el-button link type="danger" size="small" @click="emit('delete', toPayable(row))">删除</el-button>
      </div>
    </div>
    <div v-if="!loading && list.length === 0" class="empty-wrap">
      <el-empty description="暂无应付账款" />
    </div>
  </div>
  ...
  <div v-else class="content-card">
    <el-table :data="list" stripe v-loading="loading" :row-class-name="rowClass">
      ...
      <el-table-column label="已付 / 未付" width="200">
        <template #default="{ row }">
          <div class="cell-stack">
            <span class="amount-success">已付 ¥{{ formatMoney(row.paid_amount) }}</span>
            <span :class="Number(row.remaining_amount) > 0 ? 'text-danger' : 'text-success'">未付 ¥{{ formatMoney(row.remaining_amount) }}</span>
          </div>
        </template>
      </el-table-column>
      <el-table-column label="付款进度" width="180">
        <template #default="{ row }">
          <el-progress :percentage="computeRate(toPayable(row))" :color="progressColor(toPayable(row))" :stroke-width="8" />
        </template>
      </el-table-column>
      <el-table-column prop="due_date" label="到期日" width="140">
        <template #default="{ row }">
          <div class="cell-stack">
            ...
            <el-tag v-if="isOverdue(toPayable(row))" type="danger" size="small" effect="plain">逾期 {{ overdueDays(toPayable(row)) }} 天</el-tag>
          </div>
        </template>
      </el-table-column>
      <el-table-column prop="payment_term" label="付款条件" width="120">
      ...
      <el-table-column label="操作" width="240" align="center" fixed="right">
        <template #default="{ row }">
          <el-button link type="primary" size="small" @click="emit('pay', toPayable(row))" :disabled="Number(row.remaining_amount) <= 0">登记付款</el-button>
          <el-button link type="primary" size="small" @click="emit('edit', toPayable(row))">编辑</el-button>
          <el-button link type="danger" size="small" @click="emit('delete', toPayable(row))">删除</el-button>
        </template>
      </el-table-column>
...
// v0.3.25 抽自 finance/Payable.vue:69-164
defineProps<{
  list: Payable[]
  loading: boolean
  viewMode: 'card' | 'table'
...
const emit = defineEmits<{
  (e: 'pay', row: Payable): void
  (e: 'edit', row: Payable): void
  (e: 'detail', row: Payable): void
  (e: 'delete', row: Payable): void
}>()

const toPayable = (row: unknown): Payable => row as Payable

function rowClass({ row }: { row: Payable }): string {
...
.pcard {
  ...
  &__supplier { font-size: 15px; font-weight: 600; color: #303133; }
