    ...
    <div class="content-card">
      <el-table v-loading="loading" :data="list" stripe border style="width: 100%">
        ...
        <el-table-column label="类型" width="100">
          <template #default="{ row }"><el-tag size="small">{{ ISSUE_TYPE_LABEL[String(row.issue_type) as any] || row.issue_type }}</el-tag></template>
        </el-table-column>
        <el-table-column label="严重" width="80" align="center">
          <template #default="{ row }"><el-tag :type="severityColor(row.severity)" size="small">{{ SEVERITY_LABEL[String(row.severity) as any] || row.severity }}</el-tag></template>
        </el-table-column>
        <el-table-column label="状态" width="120" align="center">
          <template #default="{ row }">
            <el-tag :type="issueStatusColor(row.status)" size="small">{{ ISSUE_STATUS_LABEL[String(row.status) as any] || row.status }}</el-tag>
        ...
        <el-table-column label="操作" width="200" align="center" fixed="right">
          <template #default="{ row }">
            ...
            <el-button v-if="row.status === 'open'" link type="success" size="small" @click="handleResolve(row as any)">解决</el-button>
            <el-button v-if="row.status === 'open'" link type="warning" size="small" @click="handleIgnore(row as any)">忽略</el-button>
            <el-button v-if="row.status === 'open' && !row.work_order_id" link type="danger" size="small" @click="handleConvert(row as any)">转工单</el-button>
          </template>
        </el-table-column>
        <template #empty>
...
import { inspection, ISSUE_STATUS_LABEL, ISSUE_TYPE_LABEL, SEVERITY_LABEL, type InspectionIssue, type InspectionIssueStatus, type InspectionSeverity } from '@/api/inspection'

const route = useRoute()
const loading = ref(false)
const list = ref<any[]>([])
const pagination = reactive({ total: 0, page: 1, per_page: 15 })
const filter = reactive<{ keyword: string; status: InspectionIssueStatus | ''; severity: InspectionSeverity | '' }>({ keyword: '', status: '', severity: '' })

const detailVisible = ref(false)
const detailLoading = ref(false)
const currentIssue = ref<InspectionIssue | null>(null)

const issueStatusColor = (s?: string): 'success' | 'primary' | 'info' | 'warning' | 'danger' => ({ open: 'danger', work_order_created: 'warning', resolved: 'success', ignored: 'info' }[s || ''] as any || 'info')
const severityColor = (s?: string): 'success' | 'primary' | 'info' | 'warning' | 'danger' => ({ low: 'info', medium: 'warning', high: 'danger', critical: 'danger' }[s || ''] as any || 'info')
...
const handleView = async (row: InspectionIssue) => {
  ...
  try {
    const r: any = await inspection.getIssue(row.id)
    currentIssue.value = r?.data ?? r
  } catch (e: any) {
...
onMounted(async () => {
  ...
  // 支持 ?view=ID 直接打开详情
  if (route.query.view) {
    const r: any = await inspection.getIssue(Number(route.query.view))
    if (r?.data) {
      currentIssue.value = r.data ?? r
      detailVisible.value = true
    }
